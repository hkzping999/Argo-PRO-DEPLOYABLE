...
# 当前脚本版本号
VERSION='2.4.0-pqc-strong-plus (2026.07.03, Reality scanner + HTTPUpgrade CDN + safer config test)'
...
PROTOCOL_LIST=("VLESS + Reality Vision" "Hysteria2" "VLESS + Reality gRPC" "VLESS + WS" "VMess + WS" "Trojan + WS" "Shadowsocks + WS" "VLESS + XHTTP HTTP/1.1 CDN" "VLESS + XHTTP HTTP/3 Direct" "Trojan Direct" "Shadowsocks 2022 Direct" "VLESS + HTTPUpgrade CDN")
NODE_TAG=(     "reality-vision"         "hysteria2" "reality-grpc"         "vless-ws"   "vmess-ws"   "trojan-ws"   "ss-ws"            "xhttp-h1.1-cdn"             "xhttp-h3-direct"             "trojan-direct" "ss2022-direct"       "httpupgrade-cdn")
...
# REALITY 独立伪装目标/SNI；为空时安装过程可输入，非交互/回车则从预设池自动选择
REALITY_TARGET=${REALITY_TARGET:-''}
REALITY_SERVER_NAMES=${REALITY_SERVER_NAMES:-''}
REALITY_DOMAIN_POOL=("addons.mozilla.org" "www.microsoft.com" "www.apple.com" "www.cloudflare.com" "www.samsung.com")
# 安装时可选检测 Reality 目标域名是否可连；失败只警告不阻断，避免误杀可用域名
REALITY_CHECK_ON_INSTALL=${REALITY_CHECK_ON_INSTALL:-'y'}
REALITY_CHECK_TIMEOUT=${REALITY_CHECK_TIMEOUT:-'4'}
# REALITY ML-DSA-65 后量子签名：由 xray mldsa65 生成 seed/verify；seed 写入服务端，verify 写入客户端 pqv=
...
# 后量子加密 / PQC defaults
# y: VLESS 使用 Xray VLESS Encryption（mlkem768x25519plus = ML-KEM-768 + X25519 hybrid）
...
E[76]="Change domain / SNI / node info (argox -d)"
C[76]="更换域名 / SNI / 节点信息 (argox -d)"
...
C[107]="优选地址 (当前：\${_val})"
E[108]="TLS / Hysteria2 SNI domain (current: \${_val})"
C[108]="TLS / Hysteria2 SNI 域名 (当前：\${_val})"
...
C[124]="选择的协议及端口次序如下:"
E[125]="Reality domain / SNI (current: \${_val})"
C[125]="Reality 伪装域名 / SNI (当前：\${_val})"
...
is_vless_protocol_letter() {
  case "$1" in b|d|e|i|j|m) return 0 ;; *) return 1 ;; esac
}
...
has_vless_protocol_selected() {
  ...
  for _t in "${REINSTALL_TAGS[@]}"; do
    [[ "$_t" =~ ^(reality-vision|reality-grpc|vless-ws|xhttp-h1.1-cdn|xhttp-h3-direct|httpupgrade-cdn)$ ]] && return 0
  done
  get_installed_protocols 2>/dev/null | grep -Eq '^(reality-vision|reality-grpc|vless-ws|xhttp-h1.1-cdn|xhttp-h3-direct|httpupgrade-cdn)$' && return 0
  return 1
}
...
url_encode() {
  ...
  if [ -x "$WORK_DIR/jq" ]; then
    ...
    python3 -c 'import sys, urllib.parse; print(urllib.parse.quote(sys.stdin.read(), safe=""))' <<< "$_s" 2>/dev/null && return 0
  fi
  printf '%s' "$_s"
}
...
is_reality_protocol_letter() {
  case "$1" in b|d) return 0 ;; *) return 1 ;; esac
}

has_reality_letters() {
  local _p
  for _p in "$@"; do
    is_reality_protocol_letter "$_p" && return 0
  done
  return 1
}
...
has_reality_tags() {
  local _t
  for _t in "$@"; do
    [[ "$_t" =~ ^(reality-vision|reality-grpc)$ ]] && return 0
  done
  return 1
}

pick_default_reality_domain() {
  local _n=${#REALITY_DOMAIN_POOL[@]} _d
  if [ "$_n" -gt 0 ]; then
    if truthy "${REALITY_CHECK_ON_INSTALL:-y}"; then
      for _d in "${REALITY_DOMAIN_POOL[@]}"; do
        check_reality_target_connectivity "${_d}:443" "$_d" && { printf '%s' "$_d"; return 0; }
      done
    fi
    printf '%s' "${REALITY_DOMAIN_POOL[$((RANDOM % _n))]}"
  else
    printf '%s' "${TLS_SERVER:-addons.mozilla.org}"
  fi
}

validate_reality_sni_name() {
  local _h="$1"
  [[ "$_h" =~ ^[A-Za-z0-9]([A-Za-z0-9-]*[A-Za-z0-9])?(\.[A-Za-z0-9]([A-Za-z0-9-]*[A-Za-z0-9])?)+$ ]] || return 1
  return 0
}

check_reality_target_connectivity() {
  local _target="$1" _sni="$2" _timeout="${REALITY_CHECK_TIMEOUT:-4}"
  parse_preferred_addr "$_target" || return 1
  local _host="$PREFERRED_ADDR" _port="$PREFERRED_PORT"
  [ -z "$_sni" ] && _sni="$_host"

  # 只做轻量 TLS ClientHello 检测：可连通即认为候选可用；不把失败作为硬错误。
  if command -v timeout >/dev/null 2>&1 && command -v openssl >/dev/null 2>&1; then
    timeout "$_timeout" openssl s_client -connect "${_host}:${_port}" -servername "$_sni" -brief </dev/null >/dev/null 2>&1 && return 0
  fi
  if command -v timeout >/dev/null 2>&1 && command -v nc >/dev/null 2>&1; then
    timeout "$_timeout" nc -z "$_host" "$_port" >/dev/null 2>&1 && return 0
  fi
  if command -v timeout >/dev/null 2>&1; then
    timeout "$_timeout" bash -c "</dev/tcp/$_host/$_port" >/dev/null 2>&1 && return 0
  fi
  return 1
}

build_reality_server_names_json() {
  local _raw="$1" _out='' _item
  IFS=',' read -r -a _items <<< "$_raw"
  for _item in "${_items[@]}"; do
    _item=$(trim_spaces "$_item")
    [ -z "$_item" ] && continue
    validate_reality_sni_name "$_item" || return 1
    [ -n "$_out" ] && _out+=','
    _out+="\"${_item}\""
  done
  [ -z "$_out" ] && return 1
...
first_reality_server_name() {
  local _first="${REALITY_SERVER_NAMES%%,*}"
  _first=$(trim_spaces "$_first")
  printf '%s' "$_first"
}

ensure_reality_defaults() {
  local _default _target_host _target_port

  [ -n "${realityTarget:-}" ] && [ -z "$REALITY_TARGET" ] && REALITY_TARGET="$realityTarget"
  [ -n "${realityServerNames:-}" ] && [ -z "$REALITY_SERVER_NAMES" ] && REALITY_SERVER_NAMES="$realityServerNames"
  [ -n "${realityMldsa65Seed:-}" ] && [ -z "$REALITY_MLDSA65_SEED" ] && REALITY_MLDSA65_SEED="$realityMldsa65Seed"
  [ -n "${realityMldsa65Verify:-}" ] && [ -z "$REALITY_MLDSA65_VERIFY" ] && REALITY_MLDSA65_VERIFY="$realityMldsa65Verify"
  [ "$REALITY_TARGET" = '__REALITY_UNSET__' ] && REALITY_TARGET=''
  [ "$REALITY_SERVER_NAMES" = '__REALITY_UNSET__' ] && REALITY_SERVER_NAMES=''

  if [ -z "$REALITY_TARGET" ]; then
    if [ -n "$REALITY_SERVER_NAMES" ]; then
      _default=$(first_reality_server_name)
    else
      _default=$(pick_default_reality_domain)
    fi
    REALITY_TARGET="${_default}:443"
  fi

  parse_preferred_addr "$REALITY_TARGET" || error " Invalid REALITY_TARGET: ${REALITY_TARGET}. Expected domain:port, e.g. addons.mozilla.org:443 "
  _target_host="$PREFERRED_ADDR"
  _target_port="$PREFERRED_PORT"
  validate_reality_sni_name "$_target_host" || error " Invalid Reality target host: ${_target_host}. Please use a valid domain name. "
  REALITY_TARGET="${_target_host}:${_target_port}"

  if [ -z "$REALITY_SERVER_NAMES" ]; then
    REALITY_SERVER_NAMES="$_target_host"
  fi
  REALITY_SERVER_NAMES_JSON=$(build_reality_server_names_json "$REALITY_SERVER_NAMES") || error " Invalid REALITY_SERVER_NAMES: ${REALITY_SERVER_NAMES}. Use comma-separated domains only. "
  REALITY_PRIMARY_SERVER_NAME=$(first_reality_server_name)

  refresh_reality_fragments
}

input_reality_domain() {
  local _default _input
  _default=$(pick_default_reality_domain)
  echo
  if [ "$L" = 'C' ]; then
    hint "请输入 Reality 独立伪装域名 / SNI，例如 www.microsoft.com"
    hint "留空则自动使用预设域名：${_default}"
    reading "Reality 域名: " _input
  else
    hint "Please enter the independent Reality camouflage domain / SNI, e.g. www.microsoft.com"
    hint "Leave blank to auto-select a preset domain: ${_default}"
    reading "Reality domain: " _input
  fi
  [ -z "$_input" ] && _input="$_default"
  REALITY_TARGET="$_input"
  REALITY_SERVER_NAMES=''
  ensure_reality_defaults
  if truthy "${REALITY_CHECK_ON_INSTALL:-y}"; then
    if check_reality_target_connectivity "$REALITY_TARGET" "$REALITY_PRIMARY_SERVER_NAME"; then
      info " Reality SNI: ${REALITY_PRIMARY_SERVER_NAME} | dest: ${REALITY_TARGET} | TLS check: ok "
    else
      warning " Reality SNI: ${REALITY_PRIMARY_SERVER_NAME} | dest: ${REALITY_TARGET} | TLS check failed; continue anyway. "
    fi
  else
    info " Reality SNI: ${REALITY_PRIMARY_SERVER_NAME} | dest: ${REALITY_TARGET} "
  fi
}

refresh_reality_fragments() {
  REALITY_MLDSA65_SEED_JSON_FRAGMENT=''
  REALITY_MLDSA65_SEED_INLINE=''
  REALITY_PQV_QUERY=''
  if [ -n "$REALITY_MLDSA65_SEED" ]; then
    REALITY_MLDSA65_SEED_JSON_FRAGMENT=",
          \"mldsa65Seed\": \"${REALITY_MLDSA65_SEED}\""
    REALITY_MLDSA65_SEED_INLINE=",\"mldsa65Seed\":\"${REALITY_MLDSA65_SEED}\""
  fi
  if [ -n "$REALITY_MLDSA65_VERIFY" ]; then
    REALITY_PQV_QUERY="&pqv=$(url_encode "$REALITY_MLDSA65_VERIFY")"
  fi
}

refresh_tls_curve_fragments() {
  TLS_CURVE_PREFERENCES_JSON_FRAGMENT=''
  TLS_CURVE_PREFERENCES_INLINE=''
  truthy "${ENABLE_TLS_PQC_CURVE:-y}" || return 0

  local _raw="${TLS_CURVE_PREFERENCES:-X25519MLKEM768,X25519}" _item _out=''
  IFS=',' read -r -a _curves <<< "$_raw"
  for _item in "${_curves[@]}"; do
    _item=$(trim_spaces "$_item")
    [ -z "$_item" ] && continue
    [[ "$_item" =~ ^[A-Za-z0-9_-]+$ ]] || continue
    [ -n "$_out" ] && _out+=','
    _out+="\"${_item}\""
  done
  [ -z "$_out" ] && return 0
  TLS_CURVE_PREFERENCES_JSON="[${_out}]"
  TLS_CURVE_PREFERENCES_JSON_FRAGMENT="\"curvePreferences\": ${TLS_CURVE_PREFERENCES_JSON},"
  TLS_CURVE_PREFERENCES_INLINE=",\"curvePreferences\":${TLS_CURVE_PREFERENCES_JSON}"
}

# 校验从 GitHub 下载的 Xray-core zip 包完整性/真实性。
# 做法与官方 XTLS/Xray-install 一致：下载同名 <url>.dgst 摘要文件，比对其中的 sha256 段。
# 关键行为：
#   - 拿不到 .dgst（网络问题/该版本不提供）时只警告放行，避免把可用安装误杀；
#   - 拿到 .dgst 但摘要不匹配时，判定为下载被篡改/损坏，删除该 zip 并返回失败，
#     调用方必须据此中止安装/更新，不能继续 unzip。
# 返回值：0=校验通过或跳过，1=校验失败（且已删除可疑文件）
verify_xray_zip() {
  local _zip="$1" _url="$2"
  [ -s "$_zip" ] || return 1
  local _dgst="${_zip}.dgst"
  wget -qO "$_dgst" "${_url}.dgst" 2>/dev/null
  if [ ! -s "$_dgst" ] || grep -q 'Not Found' "$_dgst" 2>/dev/null; then
    warning " 无法获取 Xray-core 官方 .dgst 校验文件，跳过完整性校验（不影响安装，但建议手动核对）。 "
    rm -f "$_dgst"
    return 0
  fi
  local _remote_sha _local_sha
  _remote_sha=$(awk -F'= ' '/256=/{print $2}' "$_dgst" | tr -d '\r\n ')
  _local_sha=$(sha256sum "$_zip" 2>/dev/null | awk '{print $1}')
  rm -f "$_dgst"
  if [ -z "$_remote_sha" ] || [ "$_remote_sha" != "$_local_sha" ]; then
    warning " Xray-core 下载文件 SHA256 校验失败（可能被篡改或下载不完整），已删除该文件。 "
    rm -f "$_zip"
    return 1
  fi
  return 0
...
test_xray_config() {
  ...
  [ ! -s "$WORK_DIR/inbound.json" ] || [ ! -s "$WORK_DIR/outbound.json" ] && return 0

  if [ -x "$WORK_DIR/jq" ]; then
    "$WORK_DIR/jq" empty "$WORK_DIR/inbound.json" >/dev/null 2>&1 || error " inbound.json is not valid JSON. "
    "$WORK_DIR/jq" empty "$WORK_DIR/outbound.json" >/dev/null 2>&1 || error " outbound.json is not valid JSON. "
  fi

  "$_xray_bin" run -test -c "$WORK_DIR/inbound.json" -c "$WORK_DIR/outbound.json" > "$TEMP_DIR/xray-config-test.log" 2>&1 || {
    warning " Xray config test failed:"
    cat "$TEMP_DIR/xray-config-test.log" 2>/dev/null || true
    error " Abort before restarting Xray. "
  }
}

generate_reality_mldsa65() {
  truthy "${ENABLE_REALITY_MLDSA65:-y}" || { refresh_reality_fragments; return 0; }
  [ -n "$REALITY_MLDSA65_SEED" ] && [ -n "$REALITY_MLDSA65_VERIFY" ] && { refresh_reality_fragments; return 0; }

  local _xray_bin="$WORK_DIR/xray"
  [ ! -x "$_xray_bin" ] && _xray_bin="$TEMP_DIR/xray"
  if [ ! -x "$_xray_bin" ]; then
    truthy "${REALITY_MLDSA65_STRICT:-n}" && error " xray binary not found; cannot generate Reality ML-DSA-65. "
    refresh_reality_fragments
    return 0
  fi

  local _out _seed _verify
  _out=$("$_xray_bin" mldsa65 2>/dev/null || true)
  _seed=$(awk -F: 'tolower($1) ~ /seed/ {gsub(/^[ \t]+|[ \t]+$/, "", $2); print $2; exit}' <<< "$_out")
  _verify=$(awk -F: 'tolower($1) ~ /verify/ {gsub(/^[ \t]+|[ \t]+$/, "", $2); print $2; exit}' <<< "$_out")

  if [ -z "$_seed" ] || [ -z "$_verify" ]; then
    if truthy "${REALITY_MLDSA65_STRICT:-n}"; then
      error " xray mldsa65 failed or returned an unknown format. Please upgrade Xray-core, or set ENABLE_REALITY_MLDSA65='n'. "
    fi
    warning " xray mldsa65 is unavailable; Reality will continue without ML-DSA-65. "
    REALITY_MLDSA65_SEED=''
    REALITY_MLDSA65_VERIFY=''
  else
    REALITY_MLDSA65_SEED="$_seed"
    REALITY_MLDSA65_VERIFY="$_verify"
  fi
  refresh_reality_fragments
}
...
prepare_vless_pqc_keys() {
  ...
  if [ -s "$CUSTOM_FILE" ]; then
    local _old_dec _old_enc _old_en _old_strict _old_prefix _old_0rtt _old_resume _old_legacy
...
check_install() {
  ...
  [[ ${STATUS[1]} = "$(text 26)" ]] && [ ! -s $WORK_DIR/xray ] && { wget -qO $TEMP_DIR/Xray.zip ${GH_PROXY}https://github.com/XTLS/Xray-core/releases/latest/download/Xray-linux-$XRAY_ARCH.zip >/dev/null 2>&1; verify_xray_zip "$TEMP_DIR/Xray.zip" "${GH_PROXY}https://github.com/XTLS/Xray-core/releases/latest/download/Xray-linux-$XRAY_ARCH.zip" >/dev/null 2>&1 && unzip -qo $TEMP_DIR/Xray.zip xray *.dat -d $TEMP_DIR >/dev/null 2>&1; }&
...
xray_variable() {
  ...
  for i in "${!INSTALL_PROTOCOLS[@]}"; do
    ...
    case "$p" in
      ...
      m) HTTPUPGRADE_PORT=$(( START_PORT + i )) ;;
    esac
  done
  ...
  # 稳定版保护：使用临时 trycloudflare.com / 未绑定固定域名时，默认仍收敛到 WS；
  # 但如果用户显式选择 Reality(b/d)，保留 Reality 直连入站，避免安装后 Reality 被静默过滤。
  # Hysteria2/XHTTP Direct/Trojan Direct/SS2022 仍需用户明确配置证书/SNI 后再启用。
  if [[ -z "$ARGO_DOMAIN" || "$ARGO_DOMAIN" =~ trycloudflare\.com$ ]]; then
    local _stable_filtered=() _p
    for _p in "${INSTALL_PROTOCOLS[@]}"; do
      [[ " $_p " =~ ^[[:space:]]*[bdefghm][[:space:]]*$ ]] && _stable_filtered+=("$_p")
    done
  ...
  if $HAS_REALITY; then
    if [ -s "$CUSTOM_FILE" ]; then
      [ -z "$REALITY_TARGET" ] && REALITY_TARGET=$(awk -F= '/^realityTarget=/{print $2}' "$CUSTOM_FILE")
      [ -z "$REALITY_SERVER_NAMES" ] && REALITY_SERVER_NAMES=$(awk -F= '/^realityServerNames=/{print $2}' "$CUSTOM_FILE")
      [ -z "$REALITY_MLDSA65_SEED" ] && REALITY_MLDSA65_SEED=$(awk -F= '/^realityMldsa65Seed=/{print $2}' "$CUSTOM_FILE")
      [ -z "$REALITY_MLDSA65_VERIFY" ] && REALITY_MLDSA65_VERIFY=$(awk -F= '/^realityMldsa65Verify=/{print $2}' "$CUSTOM_FILE")
    fi
    if [ -z "$REALITY_TARGET" ] && ! grep -q 'noninteractive_install' <<< "$NONINTERACTIVE_INSTALL"; then
      (( STEP_NUM++ )) || true
      input_reality_domain
    else
      ensure_reality_defaults
  ...
  for p in "${INSTALL_PROTOCOLS[@]}"; do
    [[ "$p" =~ ^[efghim]$ ]] && _HAS_WS_XHTTP=true && break
  done
  for p in "${INSTALL_PROTOCOLS[@]}"; do
...
fetch_nodes_value() {
  unset SERVER_IP REALITY_PORT REALITY_PUBLIC REALITY_PRIVATE REALITY_TARGET REALITY_SERVER_NAMES REALITY_SERVER_NAMES_JSON REALITY_PRIMARY_SERVER_NAME REALITY_MLDSA65_SEED REALITY_MLDSA65_VERIFY TLS_SERVER SERVER SERVER_PORT SERVER_DISPLAY UUID WS_PATH NODE_NAME SS_METHOD SS2022_PASSWORD \
        ENABLE_REALITY_MLDSA65 REALITY_MLDSA65_STRICT ENABLE_TLS_PQC_CURVE TLS_CURVE_PREFERENCES ENABLE_VLESS_PQC VLESS_PQC_STRICT VLESS_PQC_DECRYPTION VLESS_PQC_ENCRYPTION VLESS_SERVER_DECRYPTION VLESS_CLIENT_ENCRYPTION VLESS_CLIENT_ENCRYPTION_QUERY \
        GRPC_PORT HY2_PORT VLESS_WS_PORT VMESS_WS_PORT TROJAN_WS_PORT SS_WS_PORT VLESS_XHTTP_PORT XHTTP_PORT TROJAN_PORT SS2022_PORT HTTPUPGRADE_PORT SERVER_IP_1 SERVER_IP_2 HY2_UP_NOW HY2_DOWN_NOW
  ...
  REALITY_MLDSA65_SEED="${realityMldsa65Seed:-}"
  REALITY_MLDSA65_VERIFY="${realityMldsa65Verify:-}"
  ENABLE_REALITY_MLDSA65="${enableRealityMldsa65:-${ENABLE_REALITY_MLDSA65:-y}}"
  REALITY_MLDSA65_STRICT="${realityMldsa65Strict:-${REALITY_MLDSA65_STRICT:-n}}"
  ENABLE_TLS_PQC_CURVE="${enableTlsPqcCurve:-${ENABLE_TLS_PQC_CURVE:-y}}"
  TLS_CURVE_PREFERENCES="${tlsCurvePreferences:-${TLS_CURVE_PREFERENCES:-X25519MLKEM768,X25519}}"
  SERVER="${cdn:-}"
  ...
  VLESS_PQC_DECRYPTION="${vlessPqcDecryption:-${VLESS_PQC_DECRYPTION:-}}"
  VLESS_PQC_ENCRYPTION="${vlessPqcEncryption:-${VLESS_PQC_ENCRYPTION:-}}"
  unset serverIp privateKey publicKey realityTarget realityServerNames realityMldsa65Seed realityMldsa65Verify enableRealityMldsa65 realityMldsa65Strict enableTlsPqcCurve tlsCurvePreferences cdn cdnPort language enableVlessPqc vlessPqcStrict vlessPqcDecryption vlessPqcEncryption
  ...
  [ -z "$REALITY_TARGET" ] && REALITY_TARGET=$(echo "$JSON" | $WORK_DIR/jq -r '[.inbounds[] | select(.streamSettings.security=="reality") | (.streamSettings.realitySettings.target // .streamSettings.realitySettings.dest)] | .[0] // empty' 2>/dev/null)
  [ -z "$REALITY_SERVER_NAMES" ] && REALITY_SERVER_NAMES=$(echo "$JSON" | $WORK_DIR/jq -r '[.inbounds[] | select(.streamSettings.security=="reality") | .streamSettings.realitySettings.serverNames[0]] | .[0] // empty' 2>/dev/null)
  [ -z "$REALITY_MLDSA65_SEED" ] && REALITY_MLDSA65_SEED=$(echo "$JSON" | $WORK_DIR/jq -r '[.inbounds[] | select(.streamSettings.security=="reality") | .streamSettings.realitySettings.mldsa65Seed] | .[0] // empty' 2>/dev/null)
  TLS_SERVER=$(echo "$JSON" | $WORK_DIR/jq -r '[.inbounds[] | select(.streamSettings.security=="tls") | (.streamSettings.tlsSettings.serverName // .streamSettings.tlsSettings.serverNames[0])] | .[0] // empty' 2>/dev/null)
  ...
  WS_PATH=$(echo "$JSON" | $WORK_DIR/jq -r '.inbounds[] | select(.streamSettings.network=="ws") | .streamSettings.wsSettings.path' 2>/dev/null | head -1 | sed 's|/||; s|-vl$||; s|-vm$||; s|-tr$||; s|-sh$||; s|-xh$||; s|-hu$||')
  [ -z "$WS_PATH" ] && WS_PATH=$(echo "$JSON" | $WORK_DIR/jq -r '.inbounds[] | select(.streamSettings.network=="httpupgrade") | .streamSettings.httpupgradeSettings.path' 2>/dev/null | head -1 | sed 's|/||; s|-hu$||')
  [ -z "$WS_PATH" ] && WS_PATH=$(echo "$JSON" | $WORK_DIR/jq -r '.inbounds[] | select(.streamSettings.network=="xhttp") | .streamSettings.xhttpSettings.path' 2>/dev/null | head -1 | sed 's|/||; s|-xh$||; s|-xh3$||')
  ...
  HTTPUPGRADE_PORT=$(echo "$JSON" | $WORK_DIR/jq -r '[.inbounds[] | select(.tag | split(" ")[-1] == "httpupgrade-cdn") | .port] | .[0] // empty' 2>/dev/null)
  ...
  if echo "$JSON" | $WORK_DIR/jq -e '.inbounds[]? | select(.streamSettings.security=="reality")' >/dev/null 2>&1; then
    ensure_reality_defaults
  else
    refresh_reality_fragments
  fi
  ...
  if [ -s "$WORK_DIR/inbound.json" ]; then
    ...
    while IFS=$'	' read -r TAG PORT; do
      ...
      case "$TAG" in
        ...
        vless-ws|vmess-ws|trojan-ws|ss-ws|xhttp-h1.1-cdn|httpupgrade-cdn) [ "$HAS_NGINX" = false ] && append_unique_port EXPOSED_TCP_PORTS "$PORT" ;;
...
json_nginx() {
  ...
  local _PORT_VL _PORT_VM _PORT_TR _PORT_SH _PORT_XH _PORT_HU
  if [ -s $WORK_DIR/inbound.json ] && [ -x $WORK_DIR/jq ]; then
    ...
    _PORT_HU=$(echo "$JSON_CLEAN" | $WORK_DIR/jq -r '[.inbounds[] | select(.tag | split(" ")[-1] == "httpupgrade-cdn") | .port] | .[0] // empty' 2>/dev/null)
  fi
  ...
  _PORT_HU=${_PORT_HU:-${HTTPUPGRADE_PORT}}
  ...
  grep -q 'httpupgrade-cdn' <<< "$PROTOCOLS_NOW" && _add_location "$(_ws_location "/${WS_PATH}-hu" "${_PORT_HU}")"
  local SUB_BLOCK
...
create_argo_tunnel() {
  ...
  [ -z "$WS_PATH" ] && [ -s "$WORK_DIR/inbound.json" ] && [ -x "$WORK_DIR/jq" ] && WS_PATH=$(grep -v '^//' "$WORK_DIR/inbound.json" | $WORK_DIR/jq -r '[.inbounds[] | select((.tag | split(" ")[-1]) == "httpupgrade-cdn") | .streamSettings.httpupgradeSettings.path] | .[0] // empty' 2>/dev/null | sed 's|^/||; s|-hu$||')
  WS_PATH="${WS_PATH:-$WS_PATH_DEFAULT}"
...
install_argox() {
  ...
  write_custom 'realityTarget' "${REALITY_TARGET:-__REALITY_UNSET__}"
  write_custom 'realityServerNames' "${REALITY_SERVER_NAMES:-__REALITY_UNSET__}"
  write_custom 'realityMldsa65Seed' "${REALITY_MLDSA65_SEED:-}"
  write_custom 'realityMldsa65Verify' "${REALITY_MLDSA65_VERIFY:-}"
  write_custom 'enableRealityMldsa65' "${ENABLE_REALITY_MLDSA65:-y}"
  write_custom 'realityMldsa65Strict' "${REALITY_MLDSA65_STRICT:-n}"
  write_custom 'enableTlsPqcCurve' "${ENABLE_TLS_PQC_CURVE:-y}"
  write_custom 'tlsCurvePreferences' "${TLS_CURVE_PREFERENCES:-X25519MLKEM768,X25519}"
...
WantedBy=multi-user.target"
  ...
  prepare_vless_pqc_keys
  if has_reality_letters "${INSTALL_PROTOCOLS[@]}"; then
    ensure_reality_defaults
    generate_reality_mldsa65
    write_custom 'realityTarget' "${REALITY_TARGET:-__REALITY_UNSET__}"
    write_custom 'realityServerNames' "${REALITY_SERVER_NAMES:-__REALITY_UNSET__}"
        ...
        BLOCK=$(cat << JSONEOF
    {
      ...
      "streamSettings": {
        ...
        "realitySettings": {
          "show": false,
          "dest": "${REALITY_TARGET}",
          "serverNames": ${REALITY_SERVER_NAMES_JSON},
...
export_list() {
  ...
  fetch_nodes_value
  vless_pqc_runtime_values
  ensure_reality_defaults 2>/dev/null || true
  refresh_reality_fragments
  local REALITY_SNI="${REALITY_PRIMARY_SERVER_NAME:-${REALITY_SERVER_NAMES%%,*}}"
  [ -z "$REALITY_SNI" ] && REALITY_SNI="${TLS_SERVER:-addons.mozilla.org}"
...
' ' ')
  ...
  grep -q 'reality-vision' <<< "$PROTOS_NOW" && _add \
    "{name: \"${NODE_NAME} ${NODE_TAG[0]}\", type: vless, server: ${SERVER_IP}, port: ${REALITY_PORT}, uuid: ${UUID}, encryption: "${VLESS_CLIENT_ENCRYPTION:-none}", network: tcp, udp: true, tls: true, servername: ${REALITY_SNI}, flow: xtls-rprx-vision, client-fingerprint: chrome, reality-opts: {public-key: ${REALITY_PUBLIC}, short-id: \"\"} }" \
    "vless://$(echo -n "auto:${UUID}@${SERVER_IP_2}:${REALITY_PORT}" | base64 -w0)?remarks=${NODE_NAME// /%20}%20${NODE_TAG[0]}&obfs=none&tls=1&peer=${REALITY_SNI}&xtls=2&pbk=${REALITY_PUBLIC}${REALITY_PQV_QUERY}" \
    "vless://${UUID}@${SERVER_IP_1}:${REALITY_PORT}?encryption=${VLESS_CLIENT_ENCRYPTION_QUERY:-none}&flow=xtls-rprx-vision&security=reality&sni=${REALITY_SNI}&fp=chrome&pbk=${REALITY_PUBLIC}${REALITY_PQV_QUERY}&type=tcp&headerType=none#${NODE_NAME// /%20}%20${NODE_TAG[0]}" \
    "{ \"type\":\"vless\", \"tag\":\"${NODE_NAME} ${NODE_TAG[0]}\", \"server\":\"${SERVER_IP}\", \"server_port\": ${REALITY_PORT}, \"uuid\":\"${UUID}\", \"flow\":\"xtls-rprx-vision\", \"packet_encoding\":\"xudp\", \"tls\":{ \"enabled\":true, \"server_name\":\"${REALITY_SNI}\", \"utls\":{ \"enabled\":true, \"fingerprint\":\"chrome\" }, \"reality\":{ \"enabled\":true, \"public_key\":\"${REALITY_PUBLIC}\", \"short_id\":\"\" } } }" \
  ...
  grep -q 'reality-grpc' <<< "$PROTOS_NOW" && _add \
    "{name: \"${NODE_NAME} ${NODE_TAG[2]}\", type: vless, server: ${SERVER_IP}, port: ${GRPC_PORT}, uuid: ${UUID}, network: grpc, udp: true, tls: true, servername: ${REALITY_SNI}, flow: , client-fingerprint: chrome, reality-opts: {public-key: ${REALITY_PUBLIC}, short-id: \"\"}, grpc-opts: {grpc-service-name: \"grpc\"} }" \
    "vless://$(echo -n "auto:${UUID}@${SERVER_IP_2}:${GRPC_PORT}" | base64 -w0)?remarks=${NODE_NAME// /%20}%20${NODE_TAG[2]}&path=grpc&obfs=grpc&tls=1&peer=${REALITY_SNI}&pbk=${REALITY_PUBLIC}${REALITY_PQV_QUERY}" \
    "vless://${UUID}@${SERVER_IP_1}:${GRPC_PORT}?security=reality&sni=${REALITY_SNI}&fp=chrome&pbk=${REALITY_PUBLIC}${REALITY_PQV_QUERY}&type=grpc&serviceName=grpc&encryption=${VLESS_CLIENT_ENCRYPTION_QUERY:-none}#${NODE_NAME// /%20}%20${NODE_TAG[2]}" \
    "{ \"type\": \"vless\", \"tag\":\"${NODE_NAME} ${NODE_TAG[2]}\", \"server\": \"${SERVER_IP}\", \"server_port\": ${GRPC_PORT}, \"uuid\": \"${UUID}\", \"packet_encoding\":\"xudp\", \"tls\": { \"enabled\": true, \"server_name\": \"${REALITY_SNI}\", \"utls\": { \"enabled\": true, \"fingerprint\": \"chrome\" }, \"reality\": { \"enabled\": true, \"public_key\": \"${REALITY_PUBLIC}\", \"short_id\": \"\" } }, \"transport\": { \"type\": \"grpc\", \"service_name\": \"grpc\" } }" \
  ...
  # httpupgrade-cdn
  grep -q 'httpupgrade-cdn' <<< "$PROTOS_NOW" && _add \
    "{name: \"${NODE_NAME} ${NODE_TAG[11]}\", type: vless, server: ${SERVER}, port: ${SERVER_PORT_NOW}, uuid: ${UUID}, encryption: \"${VLESS_CLIENT_ENCRYPTION:-none}\", udp: true, tls: true, servername: ${ARGO_DOMAIN}, skip-cert-verify: false, network: httpupgrade, httpupgrade-opts: {path: \"/${WS_PATH}-hu\", host: ${ARGO_DOMAIN}} }" \
    "vless://${UUID}@${SERVER}:${SERVER_PORT_NOW}?encryption=${VLESS_CLIENT_ENCRYPTION_QUERY:-none}&security=tls&type=httpupgrade&host=${ARGO_DOMAIN}&path=/${WS_PATH}-hu&sni=${ARGO_DOMAIN}#${NODE_NAME// /%20}%20${NODE_TAG[11]}" \
    "vless://${UUID}@${SERVER}:${SERVER_PORT_NOW}?encryption=${VLESS_CLIENT_ENCRYPTION_QUERY:-none}&security=tls&sni=${ARGO_DOMAIN}&fp=chrome&type=httpupgrade&host=${ARGO_DOMAIN}&path=%2F${WS_PATH}-hu#${NODE_NAME// /%20}%20${NODE_TAG[11]}" \
    "{ \"type\":\"vless\", \"tag\":\"${NODE_NAME} ${NODE_TAG[11]}\", \"server\":\"${SERVER}\", \"server_port\":${SERVER_PORT_NOW}, \"uuid\":\"${UUID}\", \"tls\": { \"enabled\":true, \"server_name\":\"${ARGO_DOMAIN}\", \"utls\": { \"enabled\":true, \"fingerprint\":\"chrome\" } }, \"transport\": { \"type\":\"httpupgrade\", \"path\":\"/${WS_PATH}-hu\", \"headers\": { \"Host\": \"${ARGO_DOMAIN}\" } } }" \
  ...
  grep -q 'xhttp-h1.1-cdn' <<< "$PROTOS_NOW" && ! grep -q 'trycloudflare\.com$' <<< "${ARGO_DOMAIN}" && _add \
    "{name: \"${NODE_NAME} ${NODE_TAG[7]}\", type: vless, server: ${SERVER}, port: ${SERVER_PORT_NOW}, uuid: ${UUID}, encryption: "${VLESS_CLIENT_ENCRYPTION:-none}", udp: true, tls: true, network: xhttp, alpn: [h2,http/1.1], servername: ${ARGO_DOMAIN}, client-fingerprint: chrome, xhttp-opts: {path: \"/${WS_PATH}-xh\", host: ${ARGO_DOMAIN}, mode: auto} }" \
    ...
    "vless://${UUID}@${SERVER}:${SERVER_PORT_NOW}?encryption=${VLESS_CLIENT_ENCRYPTION_QUERY:-none}&security=tls&sni=${ARGO_DOMAIN}&fp=chrome&alpn=h2%2Chttp%2F1.1&type=xhttp&host=${ARGO_DOMAIN}&path=%2F${WS_PATH}-xh&mode=auto#${NODE_NAME// /%20}%20${NODE_TAG[7]}" \
    "" ""
  ...
  if ! grep -Eq '^[[:space:]]*(xhttp-h1\.1-cdn|xhttp-h3-direct)[[:space:]]*$' <<< "$PROTOS_NOW" || grep -Eq '(^|[[:space:]])(reality-vision|hysteria2|reality-grpc|vless-ws|httpupgrade-cdn|vmess-ws|trojan-ws|ss-ws|trojan-direct|ss2022-direct)([[:space:]]|$)' <<< "$PROTOS_NOW"; then
...
change_protocols() {
  ...
  for tag in "${REINSTALL_TAGS[@]}"; do
    ...
    if [ -n "$_EXIST_PORT" ]; then
      ...
      case "$tag" in
        ...
        httpupgrade-cdn) HTTPUPGRADE_PORT=$_EXIST_PORT ;;
      esac
    fi
  done
  ...
  for tag in "${REINSTALL_TAGS[@]}"; do
    ...
    if [ -z "$_EXIST_PORT" ]; then
      ...
      case "$tag" in
        ...
        httpupgrade-cdn) HTTPUPGRADE_PORT=$_NEW_PORT ;;
      esac
    fi
  done
  ...
  for _t in "${ADD_TAGS[@]}"; do
    [[ "$_t" =~ ^(vless-ws|vmess-ws|trojan-ws|ss-ws|xhttp-h1.1-cdn|httpupgrade-cdn)$ ]] && _HAS_WS_XHTTP_ADD=true && break
  done
  ...
  if $_HAS_REALITY_FINAL; then
    ...
    REALITY_PRIVATE='__KEY_UNSET__'; REALITY_PUBLIC='__KEY_UNSET__'
    REALITY_TARGET='__REALITY_UNSET__'; REALITY_SERVER_NAMES='__REALITY_UNSET__'
    REALITY_MLDSA65_SEED=''; REALITY_MLDSA65_VERIFY=''
    refresh_reality_fragments
  fi
  refresh_tls_curve_fragments
  ...
  for _t in "${REINSTALL_TAGS[@]}"; do
    [[ "$_t" =~ ^(vless-ws|vmess-ws|trojan-ws|ss-ws|xhttp-h1.1-cdn|httpupgrade-cdn)$ ]] && _HAS_WS_XHTTP_FINAL=true && break
  done
...
EOF
  ...
  for tag in "${REINSTALL_TAGS[@]}"; do
    local NEW_BLOCK=''
    case "$tag" in
      hysteria2) NEW_BLOCK="{\"tag\":\"${NODE_NAME} ${NODE_TAG[1]}\",\"protocol\":\"hysteria\",\"port\":${HY2_PORT},\"settings\":{\"version\":2,\"clients\":[{\"auth\":\"${UUID}\"}]},\"streamSettings\":{\"network\":\"hysteria\",\"security\":\"tls\",\"tlsSettings\":{\"serverNames\":[\"${TLS_SERVER}\"]${TLS_CURVE_PREFERENCES_INLINE},\"alpn\":[\"h3\"],\"certificates\":[{\"certificateFile\":\"${WORK_DIR}/cert/cert.pem\",\"keyFile\":\"${WORK_DIR}/cert/private.key\"}]}}}" ;;
      ...
      xhttp-h3-direct) NEW_BLOCK="{\"tag\":\"${NODE_NAME} ${NODE_TAG[8]}\",\"port\":${XHTTP_PORT},\"protocol\":\"vless\",\"settings\":{\"clients\":[{\"id\":\"${UUID}\"}],\"decryption\":\"${VLESS_SERVER_DECRYPTION:-none}\"},\"streamSettings\":{\"network\":\"xhttp\",\"security\":\"tls\",\"xhttpSettings\":{\"mode\":\"stream-up\",\"extra\":{\"alpn\":[\"h3\"]},\"path\":\"/${WS_PATH}-xh3\"},\"tlsSettings\":{\"serverName\":\"${TLS_SERVER}\"${TLS_CURVE_PREFERENCES_INLINE},\"alpn\":[\"h3\"],\"certificates\":[{\"certificateFile\":\"${WORK_DIR}/cert/cert.pem\",\"keyFile\":\"${WORK_DIR}/cert/private.key\"}]}},\"sniffing\":{\"enabled\":true,\"destOverride\":[\"http\",\"tls\",\"quic\"]}}" ;;
      trojan-direct) NEW_BLOCK="{\"port\":${TROJAN_PORT},\"protocol\":\"trojan\",\"tag\":\"${NODE_NAME} ${NODE_TAG[9]}\",\"settings\":{\"clients\":[{\"password\":\"${UUID}\"}]},\"streamSettings\":{\"network\":\"tcp\",\"security\":\"tls\",\"tlsSettings\":{\"serverName\":\"${TLS_SERVER}\"${TLS_CURVE_PREFERENCES_INLINE},\"certificates\":[{\"certificateFile\":\"${WORK_DIR}/cert/cert.pem\",\"keyFile\":\"${WORK_DIR}/cert/private.key\"}]}},\"sniffing\":{\"enabled\":true,\"destOverride\":[\"http\",\"tls\",\"quic\"],\"metadataOnly\":false}}" ;;
      ...
      httpupgrade-cdn) NEW_BLOCK="{\"port\":${HTTPUPGRADE_PORT},\"listen\":\"127.0.0.1\",\"protocol\":\"vless\",\"tag\":\"${NODE_NAME} ${NODE_TAG[11]}\",\"settings\":{\"clients\":[{\"id\":\"${UUID}\",\"level\":0}],\"decryption\":\"${VLESS_SERVER_DECRYPTION:-none}\"},\"streamSettings\":{\"network\":\"httpupgrade\",\"security\":\"none\",\"httpupgradeSettings\":{\"path\":\"/${WS_PATH}-hu\",\"host\":\"${ARGO_DOMAIN}\",\"headers\":{\"Host\":\"${ARGO_DOMAIN}\"}}},\"sniffing\":{\"enabled\":true,\"destOverride\":[\"http\",\"tls\",\"quic\"],\"metadataOnly\":false}}" ;;
      reality-vision) NEW_BLOCK="{\"tag\":\"${NODE_NAME} ${NODE_TAG[0]}\",\"protocol\":\"vless\",\"port\":${REALITY_PORT},\"settings\":{\"clients\":[{\"id\":\"${UUID}\",\"flow\":\"xtls-rprx-vision\"}],\"decryption\":\"${VLESS_SERVER_DECRYPTION:-none}\"},\"streamSettings\":{\"network\":\"tcp\",\"security\":\"reality\",\"realitySettings\":{\"show\":false,\"dest\":\"${REALITY_TARGET}\",\"xver\":0,\"serverNames\":${REALITY_SERVER_NAMES_JSON},\"privateKey\":\"${REALITY_PRIVATE}\",\"shortIds\":[\"\"]${REALITY_MLDSA65_SEED_INLINE}}},\"sniffing\":{\"enabled\":true,\"destOverride\":[\"http\",\"tls\"]}}" ;;
      reality-grpc) NEW_BLOCK="{\"port\":${GRPC_PORT},\"protocol\":\"vless\",\"tag\":\"${NODE_NAME} ${NODE_TAG[2]}\",\"settings\":{\"clients\":[{\"id\":\"${UUID}\",\"flow\":\"\"}],\"decryption\":\"${VLESS_SERVER_DECRYPTION:-none}\"},\"streamSettings\":{\"network\":\"grpc\",\"security\":\"reality\",\"realitySettings\":{\"show\":false,\"dest\":\"${REALITY_TARGET}\",\"xver\":0,\"serverNames\":${REALITY_SERVER_NAMES_JSON},\"privateKey\":\"${REALITY_PRIVATE}\",\"shortIds\":[\"\"]${REALITY_MLDSA65_SEED_INLINE}},\"grpcSettings\":{\"serviceName\":\"grpc\",\"multiMode\":true}},\"sniffing\":{\"enabled\":true,\"destOverride\":[\"http\",\"tls\"]}}" ;;
    esac
...
change_config() {
  ...
  [ -n "$REALITY_PRIMARY_SERVER_NAME" ] && MENU_IDX+=(125) && MENU_KEY+=(reality_sni) && MENU_VAL+=("$REALITY_PRIMARY_SERVER_NAME")
  [ -n "$TLS_SERVER" ] && MENU_IDX+=(108) && MENU_KEY+=(tls_sni) && MENU_VAL+=("$TLS_SERVER")
...
"
 ...
 $(text 72) " CUSTOM_CDN
  ...
  elif [ "$KEY" = "tls_sni" ]; then
    ssl_certificate "$NEW_VAL"
  elif [ "$KEY" = "reality_sni" ]; then
    REALITY_TARGET="$NEW_VAL"
    REALITY_SERVER_NAMES=''
    ensure_reality_defaults
    NEW_VAL="$REALITY_PRIMARY_SERVER_NAME"
  ...
  case "$KEY" in
    ...
    uuid)
      ...
      ;;
    reality_sni)
      # Reality 独立 SNI：只更新 security=reality 的 dest/serverNames，不再污染 Hysteria2/TLS
      if [ -s "$_IB" ] && [ -x "$WORK_DIR/jq" ]; then
        grep -v '^//' "$_IB"           | $WORK_DIR/jq --arg dest "$REALITY_TARGET" --argjson names "$REALITY_SERVER_NAMES_JSON"               '.inbounds |= map(
      ...
      write_custom 'realityTarget' "$REALITY_TARGET"
      write_custom 'realityServerNames' "$REALITY_SERVER_NAMES"
      ;;
    tls_sni)
      # TLS/Hysteria2 SNI：只更新 security=tls 的 serverName/serverNames，不再修改 Reality
      if [ -s "$_IB" ] && [ -x "$WORK_DIR/jq" ]; then
        grep -v '^//' "$_IB"           | $WORK_DIR/jq --arg new "$NEW_VAL"               '.inbounds |= map(
                if (.streamSettings.security? == "tls") then
                  (if .streamSettings.tlsSettings.serverNames? != null then .streamSettings.tlsSettings.serverNames = [$new] else . end)
                  | (if .streamSettings.tlsSettings.serverName? != null then .streamSettings.tlsSettings.serverName = $new else . end)
                else . end
              )'           > "$_IB_TMP" && mv "$_IB_TMP" "$_IB"
      fi
      ;;
...
version() {
  ...
  if [ "${UPDATE[1],,}" = 'y' ]; then
    ...
    if [ -s $TEMP_DIR/Xray-linux-$XRAY_ARCH.zip ] && verify_xray_zip "$TEMP_DIR/Xray-linux-$XRAY_ARCH.zip" "${GH_PROXY}https://github.com/XTLS/Xray-core/releases/latest/download/Xray-linux-$XRAY_ARCH.zip"; then
      cmd_systemctl disable xray
...
menu_setting() {
  ...
  if [[ "${STATUS[*]}" =~ ($(text 27)|$(text 28)) ]]; then
    ...
    ACTION[3]() { warning " Safe edition: remote third-party script execution is disabled."; exit; }
...
exit 2
  ...
  echo ""
  error " Safe edition: legacy remote downgrade is disabled. Please reinstall with this local safe script."
  exit $?
fi
